...
export const getValue = (...path) => state =>
  prop(...path).get(state);

const addValueAndSetValue = component => {
  const getValueFromStore = ({ path, getState }) =>
    getValue(...path)(getState());
  ...
  class Form extends Component {
    ...
    subscribeToStoreChanges(subscribe) {
      ...
      this.unsubscribe = subscribe(() => {
        const value = getValueFromStore(this.props);
    ...
    render() {
      ...
      this.value = getValueFromStore({ path, getState });
